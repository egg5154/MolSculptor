...
def infer(args):
    ...
    def reward_function(molecule_dict, cached_dict, save_path=args.save_path):
        ...
        if todo_unique_smiles.size > 0:
            ...
            todo_unique_scores = []
            gen_pdbqt = True
            assert len(args.on_target_scripts) == 1
            ...
            for script in args.off_target_scripts:
                r_dock = dsdp_batch_reward(
                    ...
                    use_obabel = False,)
                r_dock = np.asarray(r_dock, np.float32) * (-1.)
                todo_unique_scores.append(todo_unique_scores[0] - r_dock)
    ...
    with open(args.init_molecule_path, 'rb') as f:
        ...
        lead_molecules['scores'][1] = lead_molecules['scores'][0] - (-1) * lead_molecules['scores'][1] # recover scores
