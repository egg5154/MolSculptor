...
python -u $SCRIPT_DIR/../../diff_opt_base.py \
    ...
    --logger_path $SAVE_DIR/opt_seh-faah_$(date "+%Y-%m-%d")_seed${1}_npop${NPOP}_nrep${NREP}_step${NSTEP}/Logs.txt \
    --save_path $SAVE_DIR/opt_seh-faah_$(date "+%Y-%m-%d")_seed${1}_npop${NPOP}_nrep${NREP}_step${NSTEP} \
    --random_seed $1 \
    --np_random_seed $1 \
    --total_step $NSTEP \
    ...
    --use_prompt \
    --n_replicate $NREP \
    --on_target_scripts $SCRIPT_DIR/dsdp_seh.sh $SCRIPT_DIR/dsdp_faah.sh \
    ...
    --sub_smiles 'O=C(NC1CCNCC1)NC2=CC=CC=C2' \
