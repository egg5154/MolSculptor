...
def main(args):
    ...
    with open(args.init_molecule_path, 'rb') as f:
        ...
        test_molecules = expand_batch_dim(NPRES, test_molecules)
    ...
    DIFFUSION_TIMESTEPS = train_config.diffusion_timesteps
    # vae config
        ...
        for i_ in range(dbs):
            ...
            for j_ in range(beam_size - 1, -1, -1):
                ...
                if smi_:
                    ...
                    # break
        return {'smiles': sanitized_output_smiles,}
    ...
    from utils import set_tfg_config, DiffusionTFG
    surrogate_model_config, tfg_config = set_tfg_config(
        score_weights = None,
        eq_steps = 10, npt = 32, dim = 16,)
    diffusion_guidance = DiffusionTFG(
        config = tfg_config,    
        dit_net = dit_net, dit_params = params,
        scheduler = scheduler, loss_fn = lambda x: x,)

    def noise_denoise(x, T, rng_key, cached, qed = None, sas = None, logp = None, mw = None):
        ## args
        params = cached['params']
        mask = cached['mask']
        eq_steps = cached['eq_steps']
        rope_index = cached['rope_index']
        ## labels & force drop ids
        bs_ = x.shape[0]
        force_drop_ids = jnp.array([1,] * bs_, jnp.int32) ## all drop
        labels = {
            'qed': jnp.array([qed if qed else -1e5] * bs_, jnp.float32),
            'sa': jnp.array([sas if sas else -1e5] * bs_, jnp.float32),
            'logp': jnp.array([logp if logp else -1e5] * bs_, jnp.float32),
            'mw': jnp.array([mw if mw else -1e5] * bs_, jnp.float32),
        }
        ## noise
        x, rng_key = diffusion_guidance.jit_noise(x, T, rng_key)
        ## denoise
        for t_d in tqdm(range(T)):
            t = T - t_d
            for _ in range(eq_steps):
                x, rng_key = diffusion_guidance.jit_denoise_step(params, x, mask, t, rope_index, rng_key, labels, force_drop_ids)
                x, rng_key = diffusion_guidance.jit_noise_step(x, t, rng_key)
            x, rng_key = diffusion_guidance.jit_denoise_step(params, x, mask, t, rope_index, rng_key, labels, force_drop_ids)
        return x
    ...
    cached = {
        ...
        'rope_index': jnp.array(
            [np.arange(npt),] * (dbs),dtype = jnp.int32).reshape(dbs, npt),
        'eq_steps': 10,
    }
    ...
    for _t in range(0, 200, 10):
        ...
        output_smiles = decoder_function(_x,)['smiles']
